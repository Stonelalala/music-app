...
class MyPage extends ConsumerWidget {
            ...
            statsAsync.when(
              data: (stats) => LayoutBuilder(
                builder: (context, constraints) {
                  ...
                  final cards = [
                    _StatCard(label: '总播放', value: '${stats.totalPlays}', icon: Icons.graphic_eq_rounded),
                    _StatCard(label: '常听歌曲', value: '${stats.uniqueTracks}', icon: Icons.music_note_rounded),
                    _StatCard(label: '收藏歌曲', value: '${stats.favoriteTracks}', icon: Icons.favorite_rounded),
                    _StatCard(label: '歌单数量', value: '${stats.playlists}', icon: Icons.queue_music_rounded),
                  ];
                  if (wide) {
                  ...
                  : Column(
                      children: playlists.map((playlist) {
                        return _PlaylistTile(
                          ...
                          onTap: () => _showPlaylistDetail(context, ref, playlist.id),
                          onLongPress: () => _showPlaylistMenu(context, ref, playlist.id, playlist.name),
                        );
                      }).toList(),
                    ),
              loading: () => const Center(child: CircularProgressIndicator()),
  ...
  Future<void> _showPlaylistDetail(BuildContext context, WidgetRef ref, String playlistId) async {
                        ...
                        child: ListView.separated(
                          ...
                          itemBuilder: (context, index) {
                            ...
                            return _TrackTile(
                              track: track,
                              onTap: () async {
                                await ref.read(playerHandlerProvider).loadQueue(
                                  detail.tracks,
                                  startIndex: index,
                                );
                                if (context.mounted) {
          ...
          children: [
            ListTile(
              ...
              onTap: () async {
                ...
                if (ok == true) {
                  await ref.read(collectionRepositoryProvider).renamePlaylist(playlistId, controller.text.trim());
                  ref.invalidate(playlistsProvider);
                  ref.invalidate(playlistDetailProvider(playlistId));
                }
              },
            ...
            ListTile(
              ...
              onTap: () async {
                Navigator.of(context).pop();
                await ref.read(collectionRepositoryProvider).deletePlaylist(playlistId);
                ref.invalidate(playlistsProvider);
                ref.invalidate(playStatsProvider);
                if (context.mounted) {
      ...
      children: displayTracks.asMap().entries.map((entry) {
        ...
        return Padding(
          ...
          child: _TrackTile(
            ...
            onTap: () async {
              await ref.read(playerHandlerProvider).loadQueue(displayTracks, startIndex: index);
              if (context.mounted) {
                context.push('/player');
              }
            },
...
class _TrackTile extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final colorScheme = Theme.of(context).colorScheme;
    final headers = auth.token == null ? null : {'Authorization': 'Bearer ${auth.token}'};
    final coverUrl = '${auth.baseUrl}/api/tracks/${track.id}/cover?auth=${auth.token}';
...
class _EmptyState extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(
        borderRadius: BorderRadius.circular(20),
        color: Theme.of(context).colorScheme.surfaceContainerHighest.withValues(alpha: 0.16),
      ),
      child: Text(
        message,
